...
- 🛠️ **`base.py` / `backup/01_Base_Controle/`**: <br>Configuração central do robô (Hub, motores de tração com giroscópio PID, garras A e B, funções de movimentação `andar()`, `girar()`, `mover_garra_a()`, `mover_garra_b()`).
...
## ⚙️ Mapeamento de Portas e Hardware

| Componente | Porta | Detalhes |
|:---|:---:|:---|
| 🏎️ **Motor de Tração Esquerdo** | 🟡 **Porta E** | Invertido *(Counterclockwise)* |
| 🏎️ **Motor de Tração Direito** | 🟡 **Porta D** | Padrão |
...
| ⚙️ **Rodas / Dimensões** | 📏 **Diâmetro:** 58.1mm | ↔️ **Distância entre rodas:** 110mm |
